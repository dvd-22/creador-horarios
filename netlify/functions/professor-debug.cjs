...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const mainSiteResponse = await axios.get('https://www.misprofesores.com', {
      timeout: 10000,
      headers: {
    ...
    console.log('Main site response size:', mainSiteResponse.data?.length || 0);

    // Try to find if there's a search endpoint by looking at the homepage
    const homepageContent = mainSiteResponse.data || '';
    const hasSearchForm = homepageContent.includes('search') || homepageContent.includes('buscar');
    const hasUniversityList = homepageContent.includes('universidades') || homepageContent.includes('escuelas');
